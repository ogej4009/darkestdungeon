...
void GameDevice::ShaderInit()
{
	...
	if (false == m_Dir.TryFindFileRecursive(L"GameShader"))
	{
		ASSERTMSG;
	}

	{
		SPtr<GameVtxShader> VTXPTR = GameVtxShader::Load(m_Dir.CreateGameFilePath(L"SpriteShader.fx"), L"VS_TEX", 5, 0);
		...
		SPtr<GamePixShader> PIXPTR = GamePixShader::Load(m_Dir.CreateGameFilePath(L"SpriteShader.fx"), L"PS_TEX", 5, 0);
	...
	//디버그 처리용 쉐이더
	{
		SPtr<GameVtxShader> VTXPTR = GameVtxShader::Load(m_Dir.CreateGameFilePath(L"DebugShader.fx"), L"VS_DEBUG", 5, 0);
		VTXPTR->LayOut.EndLayOut("POSITION", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32A32_FLOAT, 0);
		VTXPTR->CB<CTransData>(L"CTransData", 0);

		SPtr<GamePixShader> PIXPTR = GamePixShader::Load(m_Dir.CreateGameFilePath(L"DebugShader.fx"), L"PS_DEBUG", 5, 0);
		PIXPTR->CB<CVector>(L"DEBUGCOLOR", 0);
	}

	//그리드 랜더용 쉐이더
	{
		SPtr<GameVtxShader> VTXPTR = GameVtxShader::Load(m_Dir.CreateGameFilePath(L"GridShader.fx"), L"VS_GRID", 5, 0);
		VTXPTR->LayOut.EndLayOut("POSITION", 0, DXGI_FORMAT::DXGI_FORMAT_R32G32B32A32_FLOAT, 0);
		VTXPTR->CB<CTransData>(L"CTransData", 0);

		SPtr<GamePixShader> PIXPTR = GamePixShader::Load(m_Dir.CreateGameFilePath(L"GridShader.fx"), L"PS_GRID", 5, 0);
